...
def denoise_raw(input_path, output_path, ground_path, model_path, black_level, white_level,filename):
    """
    ...
    plt.savefig('data/myplot/'+filename[:-4]+'.png')
    return Score
...
def main(args):
    ...
    input_filename = sorted(os.listdir(input_path))
    ground_filename = sorted(os.listdir(ground_path))
    
    score = []
    
    for file_index in range(len(input_filename)):
        
        input_path1 = input_path + input_filename[file_index]
        output_path1 = output_path + input_filename[file_index]
        ground_path1 = ground_path + ground_filename[file_index]
        print(input_path1)
        print(ground_path1)
        
        Score = denoise_raw(input_path1, output_path1, ground_path1, model_path, black_level, white_level,input_filename[file_index])
        score.append(Score)
    
    print(np.array(score).mean().item())


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_path', type=str, default="./demo_code/save_model/model.pth")
    ...
    parser.add_argument('--output_path', type=str, default="./data/result/data/")
